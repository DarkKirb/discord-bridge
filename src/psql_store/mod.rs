...
use matrix_sdk::{async_trait, RoomInfo, StateChanges};
...
impl StateStore for PostgresStateStore {
    ...
    async fn save_filter(&self, filter_name: &str, filter_id: &str) -> StateResult<()> {
        Ok(self.save_filter(filter_name, filter_id).await?)
    }
    ...
    async fn room_timeline(
        &self,
        room_id: &RoomId,
    ) -> StateResult<Option<(BoxStream<StateResult<SyncRoomEvent>>, Option<String>)>> {
        todo!();
    }
}
